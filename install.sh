...
if ! command -v go &>/dev/null; then
    ...
    sudo apt install -y golang-go 2>/dev/null || sudo apt install -y golang &>/dev/null
fi
...
pip install wafw00f rich arjun --break-system-packages 2>/dev/null \
...
if ! command -v go &>/dev/null; then
    ...
    # Core tools (sudah ada sebelumnya)
...
for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    ...
    if grep -q "alias recon=" "$rc"; then
        # Update alias jika path berubah
